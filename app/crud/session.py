...
class SessionCRUD:
    ...
    def get_sessions(db: Session, skip: int = 0, limit: int = 100) -> List[Session]:
        return db.query(Session).filter(Session.is_active == True).offset(skip).limit(limit).all()

    @staticmethod
    def get_sessions_by_course(db: Session, course_id: UUID, skip: int = 0, limit: int = 100) -> List[Session]:
        return db.query(Session).filter(Session.course_id == course_id, Session.is_active == True).offset(skip).limit(limit).all()
